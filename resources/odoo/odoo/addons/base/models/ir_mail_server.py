...
if OpenSSL and parse_version(OpenSSL.__version__) >= parse_version('24.3.0'):
    ...
    # OpenSSL not available - define dummy functions
    def load_pem_private_key(pem_key, password):
        raise ImportError("PyOpenSSL is not installed")
...
class IrMailServer(models.Model):
    ...
    def connect(self, host=None, port=None, user=None, password=None, encryption=None,
        ...
        if mail_server and mail_server.smtp_authentication != "cli":
            ...
            if mail_server.smtp_authentication == "certificate":
                if not PyOpenSSLContext:
                    raise UserError(_('PyOpenSSL is not installed. Certificate authentication is not available.'))
                try:
