...
function validateResources() {
    const requiredPaths = [
        { path: path.join(CONFIG.resourcesPath, 'postgresql-15', 'bin', 'postgres.exe'), name: 'PostgreSQL' },
        { path: path.join(CONFIG.resourcesPath, 'python-3.10', 'python.exe'), name: 'Python' },
        { path: path.join(CONFIG.resourcesPath, 'odoo', 'odoo-bin'), name: 'Odoo' },
        { path: path.join(CONFIG.resourcesPath, 'odoo', 'run_odoo.py'), name: 'Odoo Runner' }
    ];
    
    const missing = [];
    for (const item of requiredPaths) {
        if (!fs.existsSync(item.path)) {
            missing.push(`${item.name}: ${item.path}`);
            log('ERROR', `Missing required resource: ${item.name} at ${item.path}`);
        } else {
    ...
    if (missing.length > 0) {
        const message = `Missing required resources:\n\n${missing.join('\n')}`;
...
async function ensureDatabase() {
    ...
    try {
        ...
        const checkUserCmd = `"${pg.psql}" -U postgres -p ${CONFIG.pgPort} -h localhost -c "SELECT 1 FROM pg_user WHERE usename='${CONFIG.pgUser}'" -t -A`;
        
        let userExists = false;
        try {
            const result = execSync(checkUserCmd, {
                env,
                windowsHide: true,
                encoding: 'utf8',
                stdio: 'pipe'
            });
            userExists = result.trim() === '1';
        } catch (error) {
            // Error checking, assume user doesn't exist
            userExists = false;
        }
        
        if (!userExists) {
            // Create user
            const createUserCmd = `"${pg.psql}" -U postgres -p ${CONFIG.pgPort} -h localhost -c "CREATE USER ${CONFIG.pgUser} WITH PASSWORD '${CONFIG.pgPassword}' CREATEDB SUPERUSER;"`;
...
async function startup() {
    try {
        updateStatus('Validating resources...');
        
        // Validate all required resources exist
        if (!validateResources()) {
            throw new Error('Required resources are missing. Please reinstall the application.');
        }
        ...
        // Just load the login page - simpler and more reliable
        mainWindow.loadURL(`http://localhost:${CONFIG.odooPort}/web/login?db=posdb`);
        ...
        mainWindow.webContents.on('did-finish-load', async () => {
            ...
            if (!loginAttempted) {
                ...
                `).catch(err => log('ERROR', 'Auto-login failed: ' + err.message));
            }
        });
